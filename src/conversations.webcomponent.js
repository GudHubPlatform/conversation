...
class GhConversations extends GhHtmlElement {
    ...
    async modelUpdated() {
        ...
        for(const index of Object.keys(model.data_model.messengers)) {
            ...
            if(messenger.enabled && messenger.bot_token) {
                await fetch('https://gudhub-server.eu.ngrok.io/conversation/set-webhook', {
                    method: 'POST',
                    headers: {
    ...
    async sendMessage(element) {
        ...
        const response = await fetch('https://gudhub-server.eu.ngrok.io/conversation/send-message', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    async getConversations() {
        ...
        for(const messenger of Object.keys(this.messengers)) {
            const response = await fetch(`https://gudhub-server.eu.ngrok.io/conversation/get-conversation?app_id=${this.app_id}&field_id=${this.field_id}&user_id=${encodeURIComponent(this.messengers[messenger].messenger_user_id)}&messenger=${messenger}`);
            try {
                ...
                if(!json.user) {
                    const userResponse = await fetch(`https://gudhub-server.eu.ngrok.io/conversation/update-messenger-user`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
