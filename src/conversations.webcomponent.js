...
class GhConversations extends GhHtmlElement {
    ...
    async sendMessage(element) {
        ...
        if(uploadInput.files.length > 0) {
            ...
            const response = await fetch('https://gudhub-node-server.ngrok.io/conversation/send-attachment', {
                method: 'POST',
                headers: {
    ...
    async getConversations() {
        ...
        for(const index of Object.keys(this.messengers)) {
            ...
            const response = await fetch(`https://gudhub-node-server.ngrok.io/conversation/get-conversation?app_id=${this.app_id}&field_id=${this.field_id}&user_id=${encodeURIComponent(this.messengers[index].messenger_user_id)}&messenger=${messenger}`);
            try {
